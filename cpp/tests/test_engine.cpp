...
TEST_CASE("Add Magnet Link", "[engine]") {
    ...
    REQUIRE(std::find(active.begin(), active.end(), "3a6ee3b1d5ba0b74ae580227fa34c08c5dcebe4c") != active.end());
...
TEST_CASE("Add Torrent File", "[engine]") {
    ...
    try {
        ...
        auto active = engine.get_active_torrents();
        REQUIRE(std::find(active.begin(), active.end(), hash) != active.end());
        engine.remove_torrent(hash, true);
...
TEST_CASE("Pause Resume and Remove Torrent", "[engine]") {
    ...
    auto it = std::find_if(states.begin(), states.end(), [&](const auto& s){ return s.info_hash == hash; });
    ...
    auto it_paused = std::find_if(all_states.begin(), all_states.end(), [&](const auto& s){ return s.info_hash == hash; });
    REQUIRE(it_paused != all_states.end());
    REQUIRE(it_paused->state == "paused");

    engine.resume_torrent(hash);
    auto resumed_state = engine.get_torrent_state(hash);
    REQUIRE(resumed_state.state != "paused");

    // Test remove_torrent with delete_files = true
    engine.remove_torrent(hash, true);
