...
void setup_settings_routes(httplib::Server& svr, torrent::Engine& engine) {
    svr.Get("/api/settings", [&](const httplib::Request&, httplib::Response& res) {
        json response = {
            {"download_limit", engine.get_download_limit()},
            {"upload_limit", engine.get_upload_limit()},
            {"proxy_type", engine.get_proxy_type()},
            {"require_encryption", engine.get_require_encryption()},
            {"listen_interfaces", engine.get_listen_interfaces()}
        };
    ...
    svr.Post("/api/settings", [&](const httplib::Request& req, httplib::Response& res) {
        try {
            ...
            if (body.contains("download_limit")) {
                engine.set_download_limit(body["download_limit"].get<int>());
            }
            if (body.contains("upload_limit")) {
                engine.set_upload_limit(body["upload_limit"].get<int>());
            }
            if (body.contains("proxy_type")) {
                int proxy_type = body["proxy_type"].get<int>();
                std::string proxy_hostname = body.value("proxy_hostname", "");
                int proxy_port = body.value("proxy_port", 0);
                engine.set_proxy(proxy_type, proxy_hostname, proxy_port);
            }
            if (body.contains("require_encryption")) {
                engine.set_encryption(body["require_encryption"].get<bool>());
            }
            if (body.contains("listen_interfaces")) {
                engine.set_listen_interfaces(body["listen_interfaces"].get<std::string>());
            }
...
int main() {
    ...
    setup_settings_routes(svr, engine);
...
void setup_engine_routes(httplib::Server& svr, torrent::Engine& engine) {
    svr.Post(R"(/api/torrents/([^/]+)/files/(\d+)/play)", [&engine](const httplib::Request& req, httplib::Response& res) {
        ...
        engine.stream_file(hash, file_index);
